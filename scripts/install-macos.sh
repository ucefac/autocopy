...
if [ ! -f "$CONFIG_DIR/autocopy.ini" ]; then
    # 从 releases 中复制默认配置
    CONFIG_URL="https://raw.githubusercontent.com/$REPO/$VERSION/configs/default-config"
    if curl -sL "$CONFIG_URL" -o "$CONFIG_DIR/autocopy.ini" 2>/dev/null; then
        echo "创建默认配置文件"
    else
        # 如果下载失败，创建基础配置
        cat > "$CONFIG_DIR/autocopy.ini" <<'EOF'
# AutoCopy Configuration
# 排除的应用列表（逗号分隔）
excluded_apps=Terminal,iTerm2,Alacritty,Kitty,WezTerm
